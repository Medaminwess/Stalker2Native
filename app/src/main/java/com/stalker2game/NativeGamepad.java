...
public class NativeGamepad extends View {
    ...
    public void setCanvas(Canvas c){midpCanvas=c;}
    ...
    private void fire(int i,boolean dn){
        if(midpCanvas==null||i>=KEYS.length)return;
        if(dn)midpCanvas.keyPressed(KEYS[i]);
        else midpCanvas.keyReleased(KEYS[i]);
    }
    private void buzz(){
